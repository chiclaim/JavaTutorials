...
今天我们来介绍下非常重要的数据结构：`红黑树`。

很多文章或书籍在介绍红黑树的时候直接上来就是红黑树的5个基本性质、插入、删除操作等。本文不是采用这样的介绍方式，在介绍红黑树之前，我们要了解红黑树是怎么发展出来的，进而就能知道为什么会有红黑树的5条基本性质。

这样的介绍方式也是《算法4》的介绍方式。这也不奇怪，《算法4》的作者 Robert Sedgewick 就是红黑树的作者之一。在介绍红黑树之前，我们先来看下`2-3树`
